...
class Bakery(House):
  def __init__(self):
    super().__init__()
    self.name = "Boulangerie"
    self.add_item(Potion("Potion de vie extra puissante", "Soin", 50))
    self.add_item(Weapon("Épée longue", Attack("Coup d'épée longue", 5, 15, 0)))

class Hotel(House):
  def __init__(self):
    super().__init__()
    self.name = "Hôtel"
    # Ajoutez des items différents
    self.add_item(Potion("Potion de vie légère", "Soin", 10))
    self.add_item(Weapon("Couteau", Attack("Coup de couteau", 2, 5, 0)))
